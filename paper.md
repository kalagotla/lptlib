...
Such a tool has not been openly available. Researchers in this area have generally written their own single-purpose trackers, which are rarely released, rarely tested, and impossible to compare against one another. `lptlib` was built to close that gap. It exposes nine drag closures through a single argument, spanning Stokes and Oseen [@oseen1927hydrodynamik] creeping flow, the Schiller-Naumann [@schiller1933drag] and Melling [@melling1997tracer] continuum corrections, a Cunningham-type slip correction [@cunningham1910slip], and the compressible and rarefied models of Henderson, Loth, Tedeschi [@tedeschi1999motion], and Subramaniam and Balachandar [@subramaniam2022particle], with temperature-dependent viscosity from Keyes' correlation or Sutherland's law [@sutherland1893viscosity]. All nine track against a common integrator, and the results reduce to a gridded field that downstream synthetic-imaging tools can consume. The intended users are experimentalists interpreting high-speed PIV data, and computational researchers studying one-way coupled particle-laden compressible flow, where the same machinery applies unchanged.
