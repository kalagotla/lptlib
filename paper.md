...
The vectorized PLOT3D reader is a supporting throughput feature. Reading a file in one buffered call and reshaping with Fortran-order strides removes the per-point Python overhead that dominates naive readers, and the approach was designed to be competitive with a compiled Fortran PLOT3D reader on the same files. To quantify this, a reproducible benchmark under `benchmarks/` reads a synthetic four-block structured grid of about 2.04 million points (24.5 MB, single precision) and reconstructs the block arrays, comparing the strided reader against an otherwise identical naive pure-Python reader that loops over grid points and against a compiled Fortran reader that uses stream access. On a warm cache, averaged over repeated reads, the strided reader took about 70 ms, the naive Python reader about 504 ms, and the Fortran reader about 57 ms. The strided reader is therefore about 7 times faster than the naive Python reader, and it stays within about a quarter of the compiled Fortran reader on the same order of magnitude despite running in interpreted Python (measured with Python 3.10 and NumPy 2.2; see `benchmarks/README.md` and the recorded `benchmarks/results/`). The full `GridIO.read_grid` call adds a double-precision padded allocation and per-block coordinate-bound metrics, which raises its time to about 159 ms on the same grid. Absolute times vary with hardware, but the single command in `benchmarks/run_benchmark.py` regenerates the numbers on any machine.
