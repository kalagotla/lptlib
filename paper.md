...
Performance in `lptlib` comes from the adaptive tracking scheme rather than from any single routine. By matching the integration step to the local deflection of each trajectory, the tracker takes large steps through smooth regions and refines only where a path turns sharply, which keeps the number of integration steps small while still resolving the steep tracer response across shocks. Because particles are independent under one-way coupling, the same scheme scales across cores through multiprocessing, thread pools, or MPI. The tracker is verified against a high-accuracy reference integration of the same particle equation of motion, which it reproduces to a relative velocity error of about 0.1 percent for both Stokes and standard sphere drag, and a physics-matched cross-code comparison against OpenFOAM parcel tracking is provided under `benchmarks/tracking/`.
