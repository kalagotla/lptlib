...
- **A twelve-model drag suite behind one argument.** `drag_model` accepts twelve values. Nine are published closures, namely `stokes`, `oseen`, `schiller-nauman`, `melling`, `cunningham`, `henderson`, `loth`, `subramaniam-balachandar`, and `tedeschi`. The other three are `melling-2` for the alternative Knudsen prefactor, the rigid-sphere standard-drag curve `sphere`, and a `zero-drag` mode for fluid tracers. Tracer response can be studied as a function of particle size, density, and drag closure without changing anything else.
